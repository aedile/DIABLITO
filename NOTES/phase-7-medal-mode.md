...
- **Boot straight into attract.** Doom's own title/demo cycle is the attract loop; the title is up
  about 2 s after power-on. (A procedural DIABLITO splash existed briefly and was removed on
  2026-09-19 at Jesse's request.) Any pad button on the title or during a demo opens the menu.
...
- **Mute: BOOT held 3 s** toggles it (like the other medals), persisted in NVS `medal/mute`,
  announced with a NESTOR-style toast ("Muted" / "Sound on") composited in the display task so it
  shows on every screen. Muted output is silence at the same
