...
static volatile int s_enc_status = -1;
...
static bool ensure_encrypted(uint16_t conn_handle)
{
    ...
    for (int attempt = 0; attempt < 2; attempt++) {
        ...
        if (xSemaphoreTake(s_enc_semaphore, pdMS_TO_TICKS(15000)) != pdTRUE) return false;
        if (s_enc_status == 0) return true;
        /* DIABLITO: encryption refused. The usual cause is a pad that was re-put into pairing mode
         * and dropped its side of the bond while we still hold ours: delete our copy and pair fresh. */
        struct ble_gap_conn_desc d;
        if (attempt == 0 && ble_gap_conn_find(conn_handle, &d) == 0) {
            ESP_LOGW(TAG, "encryption failed (%d): dropping stale bond and pairing again", s_enc_status);
...
esp_hidh_gattc_event_handler(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
        ...
        s_enc_status = event->enc_change.status;
        if (s_enc_semaphore) xSemaphoreGive(s_enc_semaphore);   /* success or failure: wake the waiter either way */
        return 0;
