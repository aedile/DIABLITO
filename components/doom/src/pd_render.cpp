...
#define USE_XIPCPY 0
#ifndef USE_CORE1_FOR_FLATS
#define USE_CORE1_FOR_FLATS 1
#endif
#ifndef USE_CORE1_FOR_REGULAR
...
static uint8_t *list_buffer;   // heap, allocated in pd_init: keeps .bss inside the short-pointer window
...
void pd_init() {
    list_buffer = (uint8_t *)malloc(LIST_BUFFER_SIZE);
    assert(list_buffer && !(((uintptr_t)list_buffer) & 3));
